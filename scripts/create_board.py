...
def draw_color_squares(colors):
    for i, color in enumerate(colors):
        pygame.draw.rect(
            ...
            (
                COLOR_SQUARE_X,
                COLOR_SQUARE_Y + i * (COLOR_SQUARE_SIZE + 10),
                COLOR_SQUARE_SIZE,
                COLOR_SQUARE_SIZE,
            ),
        )
...
def draw_color_indicator(selected_color):
    ...
    pygame.draw.rect(
        screen, BLACK, (INDICATOR_X, INDICATOR_Y, INDICATOR_WIDTH, INDICATOR_HEIGHT), 1
    )


def draw_save_button():
    pygame.draw.rect(
        screen, GRAY, (SAVE_BUTTON_X, SAVE_BUTTON_Y, BUTTON_WIDTH, BUTTON_HEIGHT)
    )
    font = pygame.font.Font(None, 36)
    text = font.render("Save", True, BLACK)
    text_rect = text.get_rect(
        center=(SAVE_BUTTON_X + BUTTON_WIDTH // 2, SAVE_BUTTON_Y + BUTTON_HEIGHT // 2)
    )
    screen.blit(text, text_rect)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if (
                    ...
                    SAVE_BUTTON_X <= x <= SAVE_BUTTON_X + BUTTON_WIDTH
                    and SAVE_BUTTON_Y <= y <= SAVE_BUTTON_Y + BUTTON_HEIGHT
                ):
                    save_board_state(output)
                else:
                    for i, color in enumerate(colors):
                        if (
                            COLOR_SQUARE_X <= x <= COLOR_SQUARE_X + COLOR_SQUARE_SIZE
                            and COLOR_SQUARE_Y + i * (COLOR_SQUARE_SIZE + 10)
                            <= y
                            <= COLOR_SQUARE_Y
                            + i * (COLOR_SQUARE_SIZE + 10)
                            + COLOR_SQUARE_SIZE
                        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", type=str, default="board_state.yaml", help="Output file"
    )
